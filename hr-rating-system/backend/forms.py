...
class UserEvaluationForm(forms.ModelForm):
    ...
    responsiveness = forms.ChoiceField(choices=responsiveness, label="Оценка ответсвенности", widget=forms.Select(attrs={
        'class': 'form-control', }))
    friendliness = forms.ChoiceField(choices=friendliness, label="Оценка дружелюбия", widget=forms.Select(attrs={
        'class': 'form-control', }))
